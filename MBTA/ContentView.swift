...
struct ContentView: View {
    ...
    private var activeSearchContent: some View {
        if viewModel.selectedRoute != nil && !viewModel.directions.isEmpty {
            ...
            // Show loading state while deep link / favorite is loading
            // to prevent the old route-selection UI from flashing
            VStack {
                Spacer()
                ProgressView()
                    .tint(.white)
                ...
                Button {
                    ...
                    HStack(spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text(viewModel.currentActivity != nil ? "Remove from lock screen" : "Live countdown on lock screen")
